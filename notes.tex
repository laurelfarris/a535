...
\noindent Also, note that there is a difference between the sample
mean, variance, etc. and the population quantities. The latter apply
...
\begin{equation*}
    ...
    \textrm{sample\ variance:\ } \equiv \frac{}{} = \frac{}{}
\end{equation*}
...
binomial distribution, which gives the probability of observing the
...
\begin{equation*}
    \textrm{mean} \equiv \int \\
    \textrm{variance} \equiv \\
\end{equation*}
...
appropriate distribution not only for counting photons, but for any
sort of counting experiment where a series of events occurs with a
known average rate, and are independent of time since the last
event.\\

\noindent What does the Poisson distribution look like? Plots for $
\mu$ = 2,$ \mu$ = 10,$ \mu$ = 10000.\\
...
an accident because of the central limit theorem: if a quantity
...
distribution because one important source of instrumental noise,
...
error function (sometimes called erf(x)), which is the integral of a
...
criterion, e.g., 5$ \sigma$ rather than a 2$ \sigma$ criterion; a 5$
\sigma$ has much higher level of significance. On the other hand, if
...
\chi^{2}_{}$ statistic, and determine how probable this value is,
given the number of points.
\begin{equation*}
    \chi^2 =
\end{equation*}
A quick estimate of the consistency of the model with the observed
data points can be made using reduced $ \chi^{2}_{}$, defined as $
\chi^{2}_{}$ divided by the degrees of freedom (number of data points
