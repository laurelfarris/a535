...
and are called interference filters. They are made by using two
...
\par When light from the different paths
...
\begin{itemize*}
    \item the spacing
    \item the index of refraction (usually changed by chaning the pressure)
    \item the tilt of the interference filter
\end{itemize*}
...
height $h$ (or $\phi^{\prime} = h/f$), we get an image of the slit,
which has width $\omega^{\prime}$ and height $h^{\prime}$,
given by
$$ h' = h\frac{f_{2}}{f_{1}} $$
...
$$
    ...
    \frac{\lambda{f_{1}}}{r\omega{f_{2}}(\textrm{d}\lambda/\textrm{d}x)}
$$
...
\end{figure}
The location of bright images is given by the \emph{grating equation}:
